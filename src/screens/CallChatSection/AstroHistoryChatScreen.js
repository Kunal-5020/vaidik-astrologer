...
const AstroHistoryChatScreen = ({ route }) => {
  ...
  // ✅ NEW: Navigate to Past Suggestions List
    ...
    return (
      ...
      >
        <View style={[styles.bubble, isMe ? styles.bubbleRight : styles.bubbleLeft]}>
            {item.isStarred && (
                <View style={styles.starIcon}><Ionicons name="star" size={10} color="#F59E0B" /></View>
            )}
            ...
            <View style={styles.metaContainer}>
                ...
                {isMe && <Ionicons name="checkmark-done" size={14} color="#34B7F1" style={{ marginLeft: 4 }} />}
            </View>
  ...
  if (loading) return <View style={[styles.container, styles.center]}><ActivityIndicator size="large" color="#00A884" /></View>;
  if (accessBlocked) return <View style={[styles.container, styles.center]}><Ionicons name="lock-closed-outline" size={60} color="#EF4444" /><Text style={styles.blockedText}>Access Restricted by User</Text></View>;
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right', 'bottom']}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
            ...
            <Image source={{ uri: userData?.profileImage || userData?.profilePicture || 'https://via.placeholder.com/40' }} style={styles.headerAvatar} />
            <View>
      ...
      <ImageBackground
        ...
        imageStyle={{ opacity: 0.15, resizeMode: 'center' }}
        resizeMode="cover"
      >
        <FlatList
          ref={flatListRef}
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 10, paddingVertical: 12, backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#E5E7EB', zIndex: 10 },
  ...
  remedyBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F59E0B', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 20, gap: 5 },
  remedyBtnText: { fontSize: 12, fontWeight: '600', color: '#fff' },

  chatBackground: { flex: 1, backgroundColor: '#ECE5DD' },
  dateSeparator: { alignItems: 'center', marginVertical: 12 },
  datePill: { backgroundColor: '#E1F3FB', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 12, elevation: 1 },
  ...
  bubble: { maxWidth: width * 0.75, paddingHorizontal: 10, paddingTop: 6, paddingBottom: 20, borderRadius: 8, elevation: 1, position: 'relative' },
  ...
  metaContainer: { position: 'absolute', bottom: 4, right: 8, flexDirection: 'row', alignItems: 'center' },
  ...
  leftTail: { position: 'absolute', top: 0, left: -8, width: 0, height: 0, borderTopWidth: 10, borderTopColor: '#FFFFFF', borderLeftWidth: 10, borderLeftColor: 'transparent' },
  rightTail: { position: 'absolute', top: 0, right: -8, width: 0, height: 0, borderTopWidth: 10, borderTopColor: '#D9FDD3', borderRightWidth: 10, borderRightColor: 'transparent' },
});
